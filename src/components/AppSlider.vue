...
<template>
  <div ref="slidesContainerEl" class="w-full overflow-x-hidden whitespace-nowrap flex flex-nowrap">
    <slot name="slides"></slot>
  </div>
  <slot name="navigation" :prev-slide="prevSlide" :next-slide="nextSlide" :to-slide="toSlide"></slot>
...
const toSlide = (slideNumber: number, behavior: ScrollBehavior = 'smooth') => {
  ...
  if(!slideEl) throw new Error(`slide element with index ${slideNumber} not found!`)
...
onMounted(() => {
  slideEls.value.push(...Array.from(slidesContainerEl.value?.children || []) as HTMLElement[]);
